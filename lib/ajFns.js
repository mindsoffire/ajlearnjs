            ...
            if (req.query.vE == scc.linkToken.vEmailLinkToken && req.query.eX == scc.linkToken.expires && !scc.emailVerified && scc.isDeleted != true) {
                ...
                if (chkNowForExpires < scc.linkToken.expires) {
                    ...
                    let veriClntUser = {
                        ...
                        "clntNotes": null,
                        "isDeleted": null,
                        "clntAgentCode": `${aj.f_rotjaF(scc.encEmailID)}@JScorp`
                    };
                    console.log({ 'req.query.rsT': req.query.rsT }); console.log(req.query.rsT != 'true');

                    if (req.query.rsT != 'true') {
                        scc.hshPW == null ? veriClntUser.subDate = new Date() : veriClntUser.clntCreatDate = new Date();
                    ...
                    fs.writeFile(`${mastrKYCFile}`, JSON.stringify(clntKYC), 'utf8', (err) => {
                        ...
                        if (req.query.rsT != 'true') {
                            ...
                            veriClntUser.astFK = sha256(aj.f_rotjaG(aj.f_rotajG(scc.encEmailID) + '==' + sha256(scc.hshPW)));
                            ...
                            veriClntUser.astFK = sha256(aj.f_rotjaG(aj.f_rotajG(scc.encEmailID) + '==' + sha256(oldHshPW)));
                            let agentRstClntUser = JSON.parse(fs.readFileSync(`./${veriClntUser.astFK}_agentFile.json`, 'utf8'));
                            agentRstClntUser[0].agent.hshPW = scc.hshPW;               // change hshPW in the new agent file.
                            veriClntUser.astFK = sha256(aj.f_rotjaG(aj.f_rotajG(scc.encEmailID) + '==' + sha256(scc.hshPW)));
                ...
                scc.linkToken = aj.f_genLinkTokenWTimeExpry();
                vURL = SERVERNPORT + `/api/verilink/?iD=${scc.encEmailID}&vE=${scc.linkToken.vEmailLinkToken}&eX=${scc.linkToken.expires}`;

                mailOptions.to = aj.f_rotjaF(scc.encEmailID);

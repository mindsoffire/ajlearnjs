...
const f_plugin = (req, res, clntKYC, tmpReg, oldHshPW,
            ...
            if (req.body.hshPW === scc.hshPW) {
                ...
                for (let [TempIndex, scc] of tmpReg.entries()) {
                    if (req.body.encEmailID === scc.encEmailID && scc.emailVerified === true && !scc.isDeleted) {
                        ...
                        let rstClnt = {};
                        rstClnt.encEmailID = req.body.encEmailID;
                        rstClnt.hshPW = req.body.rstPW ? req.body.rstPW : null;
                        rstClnt.emailVerified = false;
                        rstClnt.linkToken = f_genLinkTokenWTimeExpry();
                        rstClnt.emailConfirmSends = 0;
                        rstClnt.emailConfirmTries = 0;
                        vURL = SERVERNPORT + `/api/verilink/?iD=${rstClnt.encEmailID}&vE=${rstClnt.linkToken.vEmailLinkToken}&eX=${rstClnt.linkToken.expires}&rsT=true`;
                        tmpReg.push(rstClnt);
                ...
                console.log({ oldHshPW, rstPW: req.body.rstPW, vURL });
    ...
    if (!foundInKYC) {
        ...
        <img id="image" src="${SERVERNPORT}/images/Rachela_Asciified.png">`;
        console.log({ foundInTmp, mailOptionsTo: mailOptions.to, vURL });
...
const f_verilink = (req, res, clntKYC, tmpReg, oldHshPW,
    transporter = nodemailer.createTransport({
        service: 'gmail',
        auth: {
            user: 'ajphonehome@gmail.com',
    ...
    for (let [TempIndex, scc] of tmpReg.entries()) {
        ...
        if (req.query.iD == scc.encEmailID) {
            ...
            if (req.query.vE == scc.linkToken.vEmailLinkToken && req.query.eX == scc.linkToken.expires && !scc.emailVerified && scc.isDeleted != true) {
                ...
                if (chkNowForExpires < scc.linkToken.expires) {
                    ...
                    let veriClnt = {
                        ...
                        "clntNotes": null,
                        "isDeleted": null,
                        "clntCode": `${f_rotjaF(scc.encEmailID)}@${orgNm}`
                    };
                    console.log({ 'req.query.rsT': req.query.rsT }); console.log(req.query.rsT != 'true');

                    if (req.query.rsT != 'true') {
                        scc.hshPW == null ? veriClnt.subDate = new Date() : veriClnt.clntCreatDate = new Date();
                        clntKYC.push(veriClnt);
                    } else if (req.query.rsT == 'true') {
                        for (let [KYCIndex, kyc] of clntKYC.entries()) {
                    ...
                    fs.writeFile(`${mastrKYCFile}`, JSON.stringify(clntKYC), 'utf8', (err) => {
                        if (err) {
                            console.log("::eror:: writing to clntKYC masterIndexHeadTable jsonDB."); // console.log(err); console.log(data);
                            return res.status(500).json({
                                status: '..it appears we have veriClnt service write fault, please try again later.'
                            });
                        }
                        console.log("::good:: wrote to clntKYC masterIndexHeadTable jsonDB.");
                        ...
                        if (req.query.rsT != 'true') {
                            let clntAstHead = [{
                                clnt: {
                                    "encEmailID": scc.encEmailID,
                                    "hshPW": scc.hshPW,
                                    "clntCreatDate": veriClnt.clntCreatDate,
                                    "clntCode": `${f_rotjaF(scc.encEmailID)}@${orgNm}`,
                                    "legalNameID": null,
                                    "mobileNumID": null
                                }
                            }];
                            clntAstHead.push(clntAstItem);
                            veriClnt.astFK = sha256(f_rotjaG(f_rotajG(scc.encEmailID) + '==' + sha256(scc.hshPW)));
                            fs.writeFile(`./${veriClnt.astFK}_agentFile.json`, JSON.stringify(clntAstHead), 'utf8', (err) => {
                                if (err) {
                                    console.log("::eror:: writing to new distributed clnt-cms jsonDB file.");
                                    return res.status(500).json({
                                        status: '..it appears we have a distributed-cms service write fault, please try again later.'
                                    });
                                }
                                console.log("::good:: wrote to new distributed clnt-cms jsonDB file.");
                            });

                        } else if (req.query.rsT == 'true') {       // Copy contents of old file into new with reset of hshPW.
                            console.log({ oldHshPW });
                            veriClnt.astFK = sha256(f_rotjaG(f_rotajG(scc.encEmailID) + '==' + sha256(oldHshPW)));
                            let rstClnt = JSON.parse(fs.readFileSync(`./${veriClnt.astFK}_agentFile.json`, 'utf8'));
                            rstClnt[0].clnt.hshPW = scc.hshPW;               // change hshPW in the new clnt file.
                            veriClnt.astFK = sha256(f_rotjaG(f_rotajG(scc.encEmailID) + '==' + sha256(scc.hshPW)));
                            fs.writeFile(`./${veriClnt.astFK}_agentFile.json`, JSON.stringify(rstClnt), 'utf8', (err) => {
                                if (err) {
                                    console.log("::eror:: writing to new, replaced distributed clnt-cms jsonDB file.");
                                    return res.status(500).json({
                                        status: '..it appears we have a distributed-cms service write fault, please try again later.'
                                    });
                                }
                                console.log("::good:: wrote to new, replaced distributed clnt-cms jsonDB file.");
                            });
                        }
            ...
            <img id="image" src="${SERVERNPORT}/images/Rachela_Asciified.png">`;
                ...
                fs.writeFile(`${tmpRegstryFile}`, JSON.stringify(tmpReg), 'utf8', (err) => {
                    if (err) {
                        console.log("::eror:: writing to temp newClnt register jsonDB."); // console.log(err); console.log(data);
                        return res.status(500).json({
                            status: '..it appears we have newClnt service write fault, please try again later.'
                        });
                    }

...
const f_capitalize = (str, type = 'none') =>
    type === 'words' ?
        ...
        :
        type === 'sentences' ?
            f_capitalize(str.trim().split('.').filter(sentence => sentence).map(sentence => sentence.trim()).map((sentence, idx, arr) => sentence[0].toUpperCase() + sentence.substring(1) + (idx === arr.length - 1 ? (arr[arr.length - 1].slice(-1).match(/[?!,]/) === null ? '.' : '') : '')).join('.  '))
            :
            type === 'allcaps' ?
...
const f_strEncDec = (str, pos = 0, encDecVal = 40967,
    ...
    (0 <= cpVal && cpVal <= 65000 && num >= -42243) ?
        [...str].map(ch => ch.replace(ch, String.fromCodePoint(ch.charCodeAt(0) + num))).join('')
        :
        ({ error: pos === 0 ? `ajEnc:err: encDecVal = ${encDecVal} is out of range 0~65000 to encrypt.` : encDecVal > 42243 ? `ajDec:err: encDecVal = ${encDecVal} is > 42243 to decrypt.` : `ajDec:err: encDecVal = ${encDecVal} is out of range 0~65000 (${cpVal}) to decrypt.` });

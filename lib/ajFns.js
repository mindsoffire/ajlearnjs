...
const f_verilink = (req, res, clntKYC, tmpReg, oldHshPW,
    ...
    clntAstHead = [{
        agent: {
            ...
            "clntAgentCode": `${f_rotjaF(req.query.iD)}@JScorp`,
            "legalNameID": null,
            "mobileNumID": null
        }
    }, {
            ...
            if (req.query.vE == scc.linkToken.vEmailLinkToken && req.query.eX == scc.linkToken.expires && !scc.emailVerified && scc.isDeleted != true) {
                ...
                if (chkNowForExpires < scc.linkToken.expires) {
                    ...
                    let veriClnt = {
                        ...
                        "clntNotes": null,
                        "isDeleted": null,
                        "clntAgentCode": `${f_rotjaF(scc.encEmailID)}@JScorp`
                    };
                    ...
                    fs.writeFile(`${mastrKYCFile}`, JSON.stringify(clntKYC), 'utf8', (err) => {
                        ...
                        if (req.query.rsT != 'true') {
                            ...
                            fs.writeFile(`./${veriClnt.astFK}_agentFile.json`, JSON.stringify(clntAstHead), 'utf8', (err) => {
                                if (err) {
                                    console.log("::eror:: writing to new distributed agent-cms jsonDB file.");
                                    return res.status(500).json({
                            ...
                            rstClnt[0].agent.hshPW = scc.hshPW;               // change hshPW in the new agent file.
                            veriClnt.astFK = sha256(f_rotjaG(f_rotajG(scc.encEmailID) + '==' + sha256(scc.hshPW)));
                            fs.writeFile(`./${veriClnt.astFK}_agentFile.json`, JSON.stringify(rstClnt), 'utf8', (err) => {
                                if (err) {
                                    console.log("::eror:: writing to new, replaced distributed agent-cms jsonDB file.");
                                    return res.status(500).json({
                                        status: '..it appears we have a distributed-cms service write fault, please try again later.'
                                    });
                                }

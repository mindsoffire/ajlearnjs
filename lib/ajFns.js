...
const trace = (descriptr, termAdmin = 'aj') => objOrFn =>
    typeof objOrFn === 'object' ?
        console.log(`\n\x1b[1m\x1b[36m${termAdmin}:: \x1b[32m${descriptr} =\x1b[0m\n\b`, JSON.stringify(objOrFn, null, ' ' + '.'.repeat(2) + '\t'), `\n\x1b[1m\x1b[33m_______________________\x1b[0m\n`)
        :
        console.log(`\n\x1b[1m\x1b[36m${termAdmin}:: \x1b[32m${descriptr} =\x1b[0m`, objOrFn, `\n\x1b[1m\x1b[33m_______________________\x1b[0m\n`);
...
const tf_getYearA = (date = Date.now(), sec = 1000, min = 60 * sec, hr = 60 * min, d = 24 * hr, daysLeft = date / d) => {
...
const f_capitalize = (str, type = 'none') =>
    type === 'words' ?
        ...
        type === 'sentences' ?
            ...
            // f_capitalize(str.trim().split('.').map(sentence => sentence.trim()[0].toUpperCase() + sentence.trim().substring(1)).join('. '))
            :
            type === 'allcaps' ?
                str.trim().split(' ').filter(word => word).join(' ').toUpperCase() : str.trim().split(' ').filter(word => word).join(' ');


const f_strnum2OrdVals = (str, divdr = '') => str.trim().split(divdr).filter(chTkn => chTkn).map(chTkn => +chTkn ? +chTkn : chTkn);  // chTkn => Number(chTkn) ? +chTkn : chTkn
...
const f_arrSort = (arr, ascDsc = -1, numFirst = true) => {
    ...
    if (ascDsc === -1 || ascDsc === 1) {
        ...
        numStr[1].sort((a, b, x = a.toLowerCase(), y = b.toLowerCase()) =>
            x === y ?       // Use x, y as rule-set to sort actual a,b, hence don't care if toLowerCase() or toUpperCase(), both works as comparison base.
                0 :
                ascDsc === -1 ?
                    x < y ? -1 : 1      // The retns of values determine how sorting is done.
...
const f_strEncDec = (str, pos = 0, encDecVal = 40967, num = pos === 0 ? encDecVal : -encDecVal, cpVal = num + str.charCodeAt(str.length - 1)) =>
    (0 <= cpVal && cpVal <= 65000 && num >= -42243) ?
        [...str].map(ch => ch.replace(ch, String.fromCodePoint(ch.charCodeAt(0) + num))).join('') : ({ error: pos === 0 ? `ajEnc:err: encDecVal = ${encDecVal} is out of range 0~65000 to encrypt.` : encDecVal > 42243 ? `ajEnc:err: encDecVal = ${encDecVal} is > 42243 to decrypt.` : `ajEnc:err: encDecVal = ${encDecVal} is out of range 0~65000 (${cpVal}) to decrypt.` });
...
const f_encDecUsrName = (str, pos = 0, encDecVal = 'rotF',
    num = typeof encDecVal === 'number' ?
        pos === 0 ? encDecVal : -encDecVal : 0, cpVal = num + str.charCodeAt(str.length - 1)
) =>
    (0 <= cpVal && cpVal <= 65000 && num >= -42243) ?
        ...
        str.indexOf('@') !== -1 ?
            ...
            str.indexOf(' ') !== -1 ?
                ...
                str
        :
        ({ error: pos === 0 ? `ajEnc:err: encDecVal = ${encDecVal} is out of range 0~65000 to encrypt.` : encDecVal > 42243 ? `ajEnc:err: encDecVal = ${encDecVal} is > 42243 to decrypt.` : `ajEnc:err: encDecVal = ${encDecVal} is out of range 0~65000 (${cpVal}) to decrypt.` });
